...
if (-not $ready) {
    ...
    # A crash loop caused by a corrupt DERIVED collection is recoverable
    # without losing anything: code/docs are regenerated by re-indexing. The
    # recovery script refuses to touch `memory`, which nothing can regenerate,
    # so running it automatically here is safe.
    $logs = (docker logs --tail 400 rememory-qdrant 2>&1 | Out-String)
    if ($logs -match 'Failed to load local shard' -or $logs -match 'Failed to load .* index') {
        if ($logs -match 'storage/collections/memory/') {
            Write-Host "      The 'memory' collection is corrupt. It cannot be rebuilt by" -ForegroundColor Yellow
            Write-Host "      re-indexing, so setup will not delete it. Restore it with:" -ForegroundColor Yellow
            Write-Host "          uv run scripts/import_memory.py"
            Fail "Qdrant cannot start: the memory collection is damaged (see above)."
        }
        Write-Host "      This is corrupt index data in a rebuildable collection." -ForegroundColor Yellow
    ...
    Ok "recovered from corrupt index data; collections will be rebuilt"
}
